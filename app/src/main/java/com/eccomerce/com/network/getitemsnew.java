...
import com.eccomerce.com.main.fragment2;
        ...
        get_data=new Thread(new Runnable() {
            ...
            public void run() {
                try{
                    ...
                    URL url=new URL("http://"+ Api.url+"ygty/getitemsnew.php");
                    Log.d("url",""+url);
                    ...
                    String post_data = URLEncoder.encode("size", "UTF-8") + "=" + URLEncoder.encode(""+ constants.size1, "UTF-8");
                    ...
                    conn.disconnect();
                    if(sb.toString().length()<20)constants.iter1=false;
                ...
                try{//bashda data basadan hemme informasiyasyny alyas
                    ...
                    for(int i=0; i<jsonArray.length();i++) {
                        ...
                        if(!constants.idfd.contains(data.optString("id"))) {
                            ...
                            m.setVip(data.optString("vip"));
                            m.setYeri(data.optString("location"));
                            m.setEamil(data.optString("email"));
                            constants.taze.add(m);
                            constants.idfd.add(data.optString("id"));
                        }}
                    fragment2.s1.sendEmptyMessage(1);
