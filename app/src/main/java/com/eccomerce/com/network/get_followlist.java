...
public class get_followlist {
   public  static String k;
    public  static int con=0;
    static String l;
    public static   Thread get_data=new Thread();
    public static void  get_Data(final String userid){
          Log.d("gelenok","ok");
        get_data=new Thread(new Runnable() {
            ...
            public void run() {
                try{
                    ...
                    URL url=new URL("http://"+ Api.url+"ygty/get_followings.php");
                    Log.d("url",""+url);
        ...
        Thread json=new Thread(new Runnable() {
            ...
            public void run() {
                try{//bashda data basadan hemme informasiyasyny alyas
                    ...
                    catmodel m;
                    constants.followings.clear();
                    for(int i=0; i<jsonArray.length();i++) {
                        JSONObject data = jsonArray.getJSONObject(i);
                            constants.followings.add(data.optString("shopid"));

                    }
                    Log.d("followings",""+constants.followings.size());
