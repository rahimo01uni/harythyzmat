        ...
        get_data=new Thread(new Runnable() {
            ...
            public void run() {
                try{
                    ...
                    URL url=new URL("http://"+ Api.url+"ygty/get_usluga.php");
                    Log.d("url",""+url);
                    ...
                    String post_data = URLEncoder.encode("size", "UTF-8") + "=" + URLEncoder.encode(""+ constants.size3, "UTF-8")+"&"+
                            URLEncoder.encode("name", "UTF-8") + "=" + URLEncoder.encode(""+ constants.name, "UTF-8");
            ...
            public void run() {
                try{//bashda data basadan hemme informasiyasyny alyas
                    ...
                    for(int i=0; i<jsonArray.length();i++) {
                        ...
                        if(!constants.idfd2.contains(data.optString("id"))) {
                            m=new model_ulg(data.optString("id"),
                                    data.optString("uid"),
                                    data.optString("name"),
                                    data.optString("rate"),
                                    ...
                                    data.optString("image4"),
                                    data.optString("content"),
                                    data.optString("number"));
                            m.setRate(data.optString("rate"));
                            m.setImg5(data.optString("image5"));
                            m.setMg6(data.optString("image6"));
                            constants.ulgslist.add(m);
                            constants.idfd2.add(data.optString("id"));
                    }}
                    Usluga.s1.sendEmptyMessage(1);
